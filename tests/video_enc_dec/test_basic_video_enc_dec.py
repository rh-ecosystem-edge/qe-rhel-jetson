...
class TestVideoEncDec:
    ...
    def test_hardware_video_acceleration(self, ssh):
        ...
        if not spec.get("video_enc").get("supported"):
          ...
          ssh.sudo("dnf install gstreamer1-plugin-openh264 -y --transient") # for openh264enc CPU encoding
          ssh.sudo("dnf install gstreamer1-plugins-good gstreamer1-plugins-bad-free -y --transient") # for h264parse
          result = ssh.sudo(
