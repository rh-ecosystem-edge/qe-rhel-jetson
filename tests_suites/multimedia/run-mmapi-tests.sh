...
#
# Adapted from: https://github.com/OE4T/meta-tegra-community/blob/97813c33460513aad02ae65509954ad32f3d26bf/recipes-test/tegra-tests/tegra-mmapi-tests/run-mmapi-tests.sh
...
run_video_dec_drm() {
    # TODO: requires physical display with DRM encoder — enable when display is available.
    # /dev/dri/card0 exists in container (CDI exposes it) but no display connected → crash.
    # local rc=0
    # echo "Running 08_video_dec_drm"
    # video_dec_drm "$SAMPLEVID" H264 || rc=1
    # return $rc
    echo "Skipping 08_video_dec_drm - requires physical display"
    return $SKIPCODE
}
...
# SKIP_TESTS env var: comma-separated list of tests to skip (e.g., "video_encode,video_cuda_enc")
# Used by test_basic_multimedia.py to skip encoder tests on hardware without encoder (e.g., Nano)
skip_test() {
    local test="$1"
    for skip in $(echo "${SKIP_TESTS:-}" | tr ',' ' '); do
        [ "$test" = "$skip" ] && return 0
    done
    return 1
}
...
for cand in $tests_to_run; do
	...
	if skip_test "$t"; then
	    echo "=== SKIP:  $t — not supported on this hardware ==="
	    testskip=$((testskip+1))
